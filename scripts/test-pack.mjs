...
const consumer = await mkdtemp(join(tmpdir(), 'exact-duration-consumer-'));
...
import * as esm from 'exact-duration';
import {createRequire} from 'node:module';
import {runChecks} from './checks.mjs';
const cjs = createRequire(import.meta.url)('exact-duration');
assert.deepEqual(Object.keys(cjs).sort(), ['DurationError', 'parseDuration', 'tryParseDuration']);
